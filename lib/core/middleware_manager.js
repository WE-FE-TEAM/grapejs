 ...
 */
middleware.getMiddleware = function(module, name){
    if( module ){
        let moduleConfig = grapeData.middleware[module];
        if( name ){
...
middleware.executeMiddleware = function(module, name, httpObj, data){
    let middlewareClass = middleware.getMiddleware(module, name);
    if( middlewareClass ){
        let obj = new middlewareClass(httpObj);
        return obj.execute(data);
    }else{
        return Promise.reject( new Error(`middleware [${module}.${name}] 不存在!!`));
    }
};
